---
layout: archive
title: "Projects"
permalink: /projects/
author_profile: true
---

{% include base_path %}

<table>
  <tr>
    <th><a href="https://chuanzhidong.github.io/about/">About Me</a></th>
    <th><a href="https://chuanzhidong.github.io/resume/">Resume</a></th>
    <th><a href="https://chuanzhidong.github.io/projects/">Projects</a></th>
    <th><a href="https://chuanzhidong.github.io/publications/">Publications</a></th>
    <th><a href="https://chuanzhidong.github.io/photovisuals/">Photos</a></th>
    <th><a href="https://www.linkedin.com/in/chuanzhidong">LinkedIn</a></th>
    <th><a href="https://scholar.google.com/citations?user=Xs_dNn4AAAAJ&hl=en&authuser=1">Google Scholar</a></th>
    <th><a href="https://www.researchgate.net/profile/Chuan_Zhi_Dong">ResearchGate</a></th>
  </tr>
</table>

**PROJECTS**
------
* ***Recent Projects***
  * May 2025: Car Deals & Feels: A Social Platform for New Car Purchases with Location Mapping [**Link**](https://www.youtube.com/watch?v=7iQaFFBpKK4)

    <figure class="project-figure">
      <img src="{{ '/images/projects/car_deals_feels/car_deals_feels_1.png' | relative_url }}" alt="Car Deals & Feels interface overview">
      <figcaption>Car Deals & Feels dashboard highlighting nearby dealers and social reviews.</figcaption>
    </figure>

    <figure class="project-figure">
      <img src="{{ '/images/projects/car_deals_feels/car_deals_feels_2.png' | relative_url }}" alt="Car Deals & Feels mobile view">
      <figcaption>Mobile view showing real-time deals shared by the community.</figcaption>
    </figure>


<!-- * **Current Initiatives**
  * _Document ongoing projects here with their goals, collaborators, and status updates._

* **Past Projects**
  * _Summarize completed projects, emphasizing outcomes, publications, or notable impact._ -->


<table>
  <tr>
    <th><a href="https://chuanzhidong.github.io/about/">About Me</a></th>
    <th><a href="https://chuanzhidong.github.io/resume/">Resume</a></th>
    <th><a href="https://chuanzhidong.github.io/projects/">Projects</a></th>
    <th><a href="https://chuanzhidong.github.io/publications/">Publications</a></th>
    <th><a href="https://chuanzhidong.github.io/photovisuals/">Photos</a></th>
    <th><a href="https://www.linkedin.com/in/chuanzhidong">LinkedIn</a></th>
    <th><a href="https://scholar.google.com/citations?user=Xs_dNn4AAAAJ&hl=en&authuser=1">Google Scholar</a></th>
    <th><a href="https://www.researchgate.net/profile/Chuan_Zhi_Dong">ResearchGate</a></th>
  </tr>
</table>
